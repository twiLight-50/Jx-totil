<template>
  <div class="cart-sku" ref="target">
    ...
    <div class="layer" v-if="visible">
      ...
      <JxButton @click="submit" v-if="!loading" type="primary" size="mini" style="margin-left:60px">确认</JxButton>
    </div>
  </div>
</template>
<script>
...
.cart-sku {
  ...
  .layer {
    ...
    box-shadow: 2px 2px 4px lighten(@JxColor,50%);
    background: #fff;
    border-radius: 4px;
    font-size: 14px;
    padding: 20px;
    &::before {
      ...
      width: 12px;
      height: 12px;
      border-left: 1px solid @JxColor;
      border-top: 1px solid @JxColor;
      position: absolute;
      left: 12px;
      top: -8px;
      background: #fff;
