  ...
  <div class="Jx-cart-page">
    <div class="container">
      <JxBread>
        ...
        <JxBreadItem>购物车</JxBreadItem>
      </JxBread>
      <div class="cart">
        <table>
          <thead>
            <tr>
              <th width="120"><JxCheckbox @change="checkAll" :modelValue="$store.getters['cart/isCheckAll']">全选</JxCheckbox></th>
          ...
          <tbody>
            ...
            <tr v-for="goods in $store.getters['cart/validList']" :key="goods.skuId">
              <td><JxCheckbox @change="($event)=>checkOne(goods.skuId, $event)" :modelValue="goods.selected" /></td>
              <td>
                <div class="goods">
              ...
              <td class="tc">
                <JxNumbox @change="$event=>updateCount(goods.skuId,$event)" :max="goods.stock" :modelValue="goods.count"/>
              </td>
              <td class="tc">
          ...
          <tbody v-if="$store.getters['cart/invalidList'].length">
            ...
            <tr v-for="goods in $store.getters['cart/invalidList']" :key="goods.skuId">
              <td><JxCheckbox style="color:#eee;" /></td>
              <td>
                <div class="goods">
                  <RouterLink :to="`/product/${goods.id}`">
                    <img :src="goods.picture" alt="">
      ...
      <div class="action">
        <div class="batch">
          <JxCheckbox @change="checkAll" :modelValue="$store.getters['cart/isCheckAll']">全选</JxCheckbox>
        ...
        <div class="total">
          ...
          <JxButton @click="checkout()" type="primary">下单结算</JxButton>
        </div>
      </div>
      <!-- 猜你喜欢 -->
      <GoodRelevant />
